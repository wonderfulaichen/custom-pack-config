    ...
    public static List<Component> createDynamicTooltip(DatapackSettings settings, String key, String baseTooltip) {
        ...
        if (locations.size() > 1) {
            ...
            if (isCtrlPressed) {
                // 按Ctrl键时显示详细位置信息
                tooltip.append("\n\n");
                tooltip.append(Component.literal("批量修改位置 (" + locations.size() + "个):").withStyle(Style.EMPTY.withColor(TextColor.parseColor("#FFFF00").result().orElseThrow())));
                ...
                for (int i = 0; i < locations.size() && i < 5; i++) {
                    ...
                    tooltip.append(locationText.withStyle(Style.EMPTY.withColor(TextColor.parseColor("#AAAAAA").result().orElseThrow())));
                }
                
                if (locations.size() > 5) {
                    tooltip.append("\n");
                    tooltip.append(Component.literal("... 还有 " + (locations.size() - 5) + " 个位置").withStyle(Style.EMPTY.withColor(TextColor.parseColor("#AAAAAA").result().orElseThrow())));
                }
            } else {
                // 未按Ctrl键时显示提示信息
                tooltip.append("\n\n");
                tooltip.append(Component.literal("按住 Ctrl 键查看 " + locations.size() + " 个修改位置").withStyle(Style.EMPTY.withColor(TextColor.parseColor("#FFFF00").result().orElseThrow())));
            }
        }
        
        return java.util.Arrays.asList(tooltip);
    }
    ...
    private static boolean isCtrlKeyPressed() {
        try {
            ...
            if (mc != null && mc.screen != null) {
                return mc.screen.hasControlDown();
            }
        } catch (Exception e) {
            // 忽略异常
        }
    ...
    public static List<Component> createGenericDynamicTooltip(String baseTooltip, String detailedInfo) {
        ...
        if (detailedInfo != null && !detailedInfo.isEmpty()) {
            ...
            if (isCtrlPressed) {
                // 按Ctrl键时显示详细信息
                tooltip.append("\n\n");
                tooltip.append(Component.literal(detailedInfo).withStyle(Style.EMPTY.withColor(TextColor.parseColor("#AAAAAA").result().orElseThrow())));
            } else {
                // 未按Ctrl键时显示提示信息
                tooltip.append("\n\n");
                tooltip.append(Component.literal("按住 Ctrl 键查看更多信息").withStyle(Style.EMPTY.withColor(TextColor.parseColor("#FFFF00").result().orElseThrow())));
            }
        }
